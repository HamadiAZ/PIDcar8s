...
void updatesensors(char S = 'B', WhichLineToFollow direction = WhichLineToFollow::AUTO, boolean AutoTopLeftOrRight = true)
{ // updatesensors( BLACK B mode or WHite W mode)
    ...
    if (S == 'W')
    {
        position = qtr.readLineWhite(sensors, QTRReadMode::On, direction, AutoTopLeftOrRight);
        currentLineColor = 'W';
    }
    else
    {
        // qtr.readLineBlack(sensors, QTRReadMode::On,  WhichLineToFollow::AUTO // RIGHT // LEFT, AutoTopLeftOrRight=true);
        position = qtr.readLineBlack(sensors, QTRReadMode::On, direction, AutoTopLeftOrRight);
        currentLineColor = 'B';
    }
    //   Serial.print("cuurent color ");Serial.print (currentLineColor); Serial.print("// Dsensors :");
    // LED PRINTING ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if ((millis() - ledactiontime) > 30) // update leds if not myledwhiteon() ( after 50ms delay)
    {
        // LED PRINTING
...
//                        int pathTimes[10]; 
            ...
            int widthOfActiveLeds = 5000; // 1000 ~ 1led
            ...
            if (brightness > 0)
            {
                if (isPathTimesArrayCorrect == true)
                {
                    ...
                    if (deltaTime < delayBetweenEachAction)
                    {
                        // still blocked
