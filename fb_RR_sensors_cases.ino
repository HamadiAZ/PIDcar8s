...
{ // compare (readings , string elli feha les X lezm tkoun 2eme parameter)
  ...
  else if (path[pathSteps] == 'l')
  {
    ...
    while (1)
    {
      left(120, 100); // ////Serial.println("right");
      updatesensors(currentLineColor);
...
{ //  right or f //
  ...
  else if (path[pathSteps] == 'r')
  {
    ...
    while (1)
    {
      right(170, 170); // ////Serial.println("right");
      updatesensors(currentLineColor);
      if (((IntDsensors[0] == 0 && IntDsensors[1] == 1 && IntDsensors[7] == 0 && IntDsensors[4] == 0 ) || (IntDsensors[1] == 0 && IntDsensors[2] == 1 && IntDsensors[0] == 0 && IntDsensors[7] == 0 )) && CountLines() <= 1)
        break;
    }
    Taction = millis();
